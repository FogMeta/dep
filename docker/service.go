...
func (ds *DockerService) PushImage(imagesName string, authConf *registry.AuthConfig) error {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*6000)
	defer cancel()
	authConfigBytes, _ := json.Marshal(authConf)
...
func (ds *DockerService) ListImages() (map[string]string, error) {
	...
	if err != nil {
		log.Printf("Unable to list image, error: %+v", err)
	}
	...
	for _, image := range imageList {
		if len(image.RepoTags) == 0 || strings.EqualFold(image.RepoTags[0], "<none>:<none>") {
			if err := ds.RemoveImage(image.ID); err != nil {
				log.Printf("Failed delete unused image, error: %+v", err)
				continue
			}
...
func (ds *DockerService) CleanResource() {
	...
	if err != nil {
		log.Printf("Failed delete dangling image, error: %+v", err)
		return
	}

	if _, err = ds.c.ContainersPrune(ctx, filters.NewArgs()); err != nil {
		log.Printf("Failed delete unused container, error: %+v", err)
		return
	}

	images, err := ds.c.ImageList(context.Background(), types.ImageListOptions{})
	if err != nil {
		log.Printf("Failed get image list, error: %+v", err)
		return
	}

	for _, image := range images {
		if image.Containers == 0 {
			log.Printf("start clean unused image, imageId: %s", image.ID)
